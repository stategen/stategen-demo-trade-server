/*
 * Do not remove this unless you get business authorization.
 * Copyright (c) 2016 - 2018 All Rights Reserved.
 * Powered By [stategen.dalgen]
 */
package com.mycompany.biz.domain;

import java.text.SimpleDateFormat;

import javax.persistence.Id;
import javax.persistence.Temporal;
import javax.persistence.TemporalType;
import javax.validation.constraints.Max;
import org.stategen.framework.lite.IOption;

import com.alibaba.fastjson.annotation.JSONField;
import io.swagger.annotations.ApiModelProperty;

import lombok.AllArgsConstructor;
import lombok.Builder;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

/**
 * City
 * <pre>
 * database table: demo_city
 * database table comments: City
 * This file is generated by <tt>dalgenx</tt>, a DAL (Data Access Layer)
 *
 * 该类仅不可以修改dalgenx生成的属生(field)类型 ,方法(Method)返回值类型和参数类型
 * 该类可自由继承、实现接口，自定义字段或方法，在下次生成时均不会丢失，可有效减少DTO的使用.
 * 同时，支持自定义修改字段大小写，一旦修改后，请再执行 gen.sh dal demo_city -e
 * 不建议使@Data,其hashCode有问题
 * 如果表修改后 ，请执行 gen.sh table demo_city -e ,再执行 gen.sh dal demo_city -e
 * </pre>
 */
@Getter
@Setter
@Builder
@NoArgsConstructor
@AllArgsConstructor
public class City implements java.io.Serializable, IOption {

    /***不使用或自定义字段在下次生成后，会移到serialVersionUID的上面*/
    private static final long serialVersionUID = -5216457518046898601L;

    /***cityId   db_column: city_id VARCHAR */
    @ApiModelProperty("cityId")
    @Id
    @Max(32)
    private String cityId;

    /***name   db_column: name VARCHAR */
    @ApiModelProperty("name")
    @Max(100)
    private String name;

    /***pycode   db_column: pycode VARCHAR */
    @ApiModelProperty("pycode")
    @Max(50)
    private String pycode;

    /***provinceId   db_column: province_id VARCHAR */
    @ApiModelProperty("provinceId")
    @Max(64)
    private String provinceId;

    /***postcode   db_column: postcode VARCHAR */
    @ApiModelProperty("postcode")
    @Max(50)
    private String postcode;

    /***areacode   db_column: areacode VARCHAR */
    @ApiModelProperty("areacode")
    @Max(50)
    private String areacode;

    /***创建时间   db_column: create_time TIMESTAMP */
    @ApiModelProperty(value = "创建时间", hidden = true)
    @Temporal(TemporalType.TIMESTAMP)
    private java.util.Date createTime;

    /***更新时间   db_column: update_time TIMESTAMP */
    @ApiModelProperty(value = "更新时间", hidden = true)
    @Temporal(TemporalType.TIMESTAMP)
    private java.util.Date updateTime;

    /***是否删除(0:正常，1删除)   db_column: delete_flag INTEGER */
    @ApiModelProperty(value = "是否删除(0:正常，1删除)", hidden = true)
    private Integer deleteFlag;

    /*----------------getter & setter ignore by lombok -------------------*/
    public String toString() {
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd H:m:ss.SSS");
        StringBuilder sb = new StringBuilder(1024);
        sb.append('{');
        sb.append("cityIds").append('=').append(cityIds).append('\n');
        sb.append("nameLike").append('=').append(nameLike).append('\n');
        sb.append("pycodeLike").append('=').append(pycodeLike).append('\n');
        sb.append("provinceIds").append('=').append(provinceIds).append('\n');
        sb.append("postcodeLike").append('=').append(postcodeLike).append('\n');
        sb.append("areacodeLike").append('=').append(areacodeLike).append('\n');
        sb.append("createTimeMin").append('=').append(createTimeMin != null ? df.format(createTimeMin) : null).append('\n');
        sb.append("createTimeMax").append('=').append(createTimeMax != null ? df.format(createTimeMax) : null).append('\n');
        sb.append("updateTimeMin").append('=').append(updateTimeMin != null ? df.format(updateTimeMin) : null).append('\n');
        sb.append("updateTimeMax").append('=').append(updateTimeMax != null ? df.format(updateTimeMax) : null).append('\n');
        sb.append("cityId").append('=').append(getCityId()).append('\n');
        sb.append("name").append('=').append(getName()).append('\n');
        sb.append("pycode").append('=').append(getPycode()).append('\n');
        sb.append("provinceId").append('=').append(getProvinceId()).append('\n');
        sb.append("postcode").append('=').append(getPostcode()).append('\n');
        sb.append("areacode").append('=').append(getAreacode()).append('\n');
        sb.append("createTime").append('=').append(getCreateTime() != null ? df.format(getCreateTime()) : null).append('\n');
        sb.append("updateTime").append('=').append(getUpdateTime() != null ? df.format(getUpdateTime()) : null).append('\n');
        sb.append("deleteFlag").append('=').append(getDeleteFlag());
        sb.append('}');
        return sb.toString();
    }

    /***查询字段下次生成时会自动移到toString()方法下面*/
    /*** cityIds in getPageList */
    @ApiModelProperty("cityId s")
    @JSONField(serialize = false)
    private java.util.List<String> cityIds;

    /*** nameLike in getPageList */
    @ApiModelProperty("nameLike")
    @JSONField(serialize = false)
    private String nameLike;

    /*** pycodeLike in getPageList */
    @ApiModelProperty("pycodeLike")
    @JSONField(serialize = false)
    private String pycodeLike;

    /*** provinceIds in getPageList */
    @ApiModelProperty("provinceId s")
    @JSONField(serialize = false)
    private java.util.List<String> provinceIds;

    /*** postcodeLike in getPageList */
    @ApiModelProperty("postcodeLike")
    @JSONField(serialize = false)
    private String postcodeLike;

    /*** areacodeLike in getPageList */
    @ApiModelProperty("areacodeLike")
    @JSONField(serialize = false)
    private String areacodeLike;

    /*** 创建时间Min in getPageList */
    @ApiModelProperty("创建时间Min")
    @Temporal(TemporalType.TIMESTAMP)
    @JSONField(serialize = false)
    private java.util.Date createTimeMin;

    /*** 创建时间Max in getPageList */
    @ApiModelProperty("创建时间Max")
    @Temporal(TemporalType.TIMESTAMP)
    @JSONField(serialize = false)
    private java.util.Date createTimeMax;

    /*** 更新时间Min in getPageList */
    @ApiModelProperty("更新时间Min")
    @Temporal(TemporalType.TIMESTAMP)
    @JSONField(serialize = false)
    private java.util.Date updateTimeMin;

    /*** 更新时间Max in getPageList */
    @ApiModelProperty("更新时间Max")
    @Temporal(TemporalType.TIMESTAMP)
    @JSONField(serialize = false)
    private java.util.Date updateTimeMax;

    @Override
    public String getTitle() {
        if (name != null) {
            return name;
        }
        return null;
    }

    @Override
    public String getValue() {
        if (cityId != null) {
            return cityId;
        }
        return null;
    }
}
